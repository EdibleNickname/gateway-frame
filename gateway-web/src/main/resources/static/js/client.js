...
$(function () {
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                if (jqXHR.status === 401 || jqXHR.status === 403) {
                    $('#loginErrorModal')
                        ...
                        .find(".modal-body")
                        .empty()
                        .html("<p>服务器返回的数据:<br>" + jqXHR.responseText + "</p>");
                } else {
    ...
    function createAuthorizationTokenHeader() {
        ...
        if (token) {
            return {"Authorization": "LCN" + token};
        } else {
            return {};
        }
    }
    ...
    function showUserInformation() {
        ...
        var str = createAuthorizationTokenHeader();
        $.ajax({
            url: "/user",
            type: "GET",
            contentType: "application/json; charset=utf-8",
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                $userInfoBody.append($("<div>").text("用户名: " + data.username));
                $userInfoBody.append($("<div>").text("邮箱: " + data.email));

                var $authorityList = $("<ul>");
                data.authorities.forEach(function (authorityItem) {
                    $authorityList.append($("<li>").text(authorityItem.authority));
                });
                var $authorities = $("<div>").text("认证:");
